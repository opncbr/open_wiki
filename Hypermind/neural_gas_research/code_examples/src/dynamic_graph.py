...
class DynamicGraph(Module):
    ...
    def __init__(self, 
        ...
        self.tensor = Parameter(Tensor(node_num, input_length))
    ...
    def add_nodes(self, 
        ...
        self.tensor = Parameter(data = cat((self.tensor, new_tensor), dim=0))
    ...
    def add_edges(self, 
        ...
        assert (U.max() <= len(self.tensor)-1) and (V.max() <= len(self.tensor)-1), "edge index exceeds node count"
    ...
    def remove_nodes(self, 
        ...
        node_mask = (arange(self.tensor.size(0)).unsqueeze(1) == node_indices).sum(1) == 0
        self.tensor = Parameter(self.tensor[node_mask,:])
