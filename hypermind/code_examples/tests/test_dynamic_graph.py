...
def test_createGraphDefault():
    ...
    assert list(dg.weight.shape) == [2,10]
...
def test_createGraphCustom():
    ...
    assert list(dg.weight.shape) == [15,10]
...
def test_add_nodes():
    ...
    assert list(dg.weight.shape) == [11,10]
...
def test_remove_nodes():
    ...
    cloned_tensor = dg.weight[left_indices,:].detach().clone()
    dg.remove_nodes(node_indices = torch.tensor(indices2remove))
    assert torch.all(torch.eq(cloned_tensor, dg.weight)).item()
...
def test_empty_graph():
    ...
    assert list(dg.weight.shape) == [0,10]
